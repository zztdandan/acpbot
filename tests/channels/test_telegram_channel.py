...
@pytest.mark.asyncio
async def test_forward_command_keeps_start_and_help_local() -> None:
    channel = TelegramChannel(
        TelegramConfig(enabled=True, token="123:abc", allow_from=["*"], group_policy="open"),
        MessageBus(),
    )
    ...
    await channel._forward_command(_make_telegram_update(text="/start@nanobot_test"), None)
    await channel._forward_command(_make_telegram_update(text="/help"), None)
